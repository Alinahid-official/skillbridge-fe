...
function Chat({ socket, username, room, passemail }) {
  ...
  const loadPrograms = async () => {
    fetch("https://sxt9335.uta.cloud/fetchRole.php", {
      method: "POST",
      headers: {
  ...
  const loadMessages = async (email1) => {
    try {
      const res = await axios.get(`${url}/messages?email1=${email}&email2=${email1}`)
      console.log(res.data)
      setDataSource(res.data.map((message) => {
            ...
            {contacts.map((contact, index) => {
              return (
                ...
                onClick={()=>{
                  ...
                  setTo(contact.email)}}
                >
                  <Avatar name={contact.first_name} size="50" round={true} />
                  <span style={{ marginLeft: '10px' }}>{contact.first_name} {contact.last_name}</span>
                </div>
              )
            })}
